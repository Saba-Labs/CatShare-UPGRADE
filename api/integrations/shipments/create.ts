...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const shipment = await createShiprocketShipmentForOrder(supabase, auth.userId, orderId);
    ...
    const status =
      msg.includes('not found') || msg.includes('delivery address') || msg.includes('Connect Shiprocket')
        ? 400
        : 500;
