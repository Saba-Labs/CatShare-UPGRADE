-- Extend coupon validation with category restrictions on cart items.

create or replace function public.order_item_matches_coupon_categories(
  p_item jsonb,
  p_allowed jsonb
)
returns boolean
...
begin
  ...
  if p_item ? 'categories' and jsonb_typeof(p_item -> 'categories') = 'array' then
    for v_cat in
      select trim(value)
      from jsonb_array_elements_text(p_item -> 'categories') as t(value)
    loop
      if v_cat <> '' and v_cat = any (v_allowed) then
        return true;
      end if;
    end loop;
  end if;

  v_cat := trim(coalesce(p_item ->> 'category', ''));
  if v_cat <> '' and v_cat = any (v_allowed) then
    return true;
  end if;
...
-- Recreate storefront order RPC to pass items into coupon validation.
