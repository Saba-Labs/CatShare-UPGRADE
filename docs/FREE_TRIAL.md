...
- Each **Supabase account** gets **30 days of full Pro access** from **`auth.users.created_at`** (account creation time).
...
- **No extra database table** is required: trial end is computed server-side as `created_at + 30 days`.
...
| `trialDays` | `30` (constant from `lib/subscriptionEntitlement.mjs`) |

Shared logic lives in **`lib/subscriptionEntitlement.mjs`** (used by Vercel and `backend/server.js`).

## Existing accounts

Users who created accounts **more than 30 days ago** will have an **expired** trial immediately (same as “trial already used” by calendar time). To grant a promotional extension later, you could add a DB column or support table—**not** implemented in this version.

## Client

- **`SubscriptionContext`** exposes `isPro`, `isPaidPro`, `isTrialActive`, `trialEndsAt`.
