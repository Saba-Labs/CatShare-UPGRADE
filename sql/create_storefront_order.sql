-- Run in Supabase SQL Editor so anonymous storefront customers can place orders.
-- Also applies the inventory fix for untracked SKUs (sql/warehouse_inventory.sql § apply_order_inventory).

-- 1) RLS: allow store checkout inserts without seller session
...
-- 2) Security-definer RPC (bypasses RLS; preferred path from the app)
